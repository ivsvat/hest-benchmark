...
from hest.bench.cpath_model_zoo.inference_models import CustomInferenceEncoder, InferenceEncoder, inf_encoder_factory
...
### optimizer settings ###
parser.add_argument("--seed", type=int, help="random seed for reproducible experiment")
...
parser.add_argument("--custom_split_folder", type=str)
parser.add_argument(
    "--dimreduce",
    type=str,
    help='whenever to perform dimensionality reduction before linear probing, can be "PCA" or None',
)
...
parser.add_argument(
    "--config",
    type=str,
    help="Path to a benchmark config file, arguments provided in the config file will overwrite the command line args",
)
...
class BenchmarkConfig:
    ...
    custom_split_folder: Optional[str] = "splits"
...
def benchmark_grid(args, device, model_names, datasets: List[str], save_dir, custom_encoder=None) -> Tuple[list, dict]:
    ...
    for dataset in datasets:
        ...
        for model_name in model_names:
            ...
            enc_results = predict_folds(
                args, exp_save_dir, model_name, dataset, device, bench_data_root, custom_encoder
            )
        ...
        dataset_perfs.append({"dataset_name": dataset, "results": enc_perfs})

    perf_per_enc = {}
    for dataset_perf in dataset_perfs:
        for enc_perf in dataset_perf["results"]:
            perf_per_enc[enc_perf["encoder_name"]] = perf_per_enc.get(enc_perf["encoder_name"], []) + [
                enc_perf["pearson_mean"]
            ]
...
def embed_tiles(dataloader: DataLoader, model: torch.nn.Module, embedding_save_path: str, device: str, precision):
...
def predict_single_split(
            ...
            if extract_tiles:
                if not os.path.isfile(embed_path) or args.overwrite:
                    ...
                    tile_dataset = H5HESTDataset(
                        tile_h5_path, chunk_size=args.batch_size, img_transform=encoder.eval_transforms
                    )
    ...
    for split_key, split in zip(["train", "test"], [train_df, test_df]):
        ...
        if split_key == "train":
            logger.info(
                f"Loaded {split_key} split from {train_split} with {len(split_assets['embeddings'])} samples: {split_assets['embeddings'].shape}"
            )
        if split_key == "train":
            logger.info(
    ...
    if args.dimreduce == "PCA":
        ...
        pipe = Pipeline(
            [("scaler", StandardScaler()), ("PCA", PCA(n_components=args.latent_dim, random_state=args.seed))]
        )
    ...
    probe_results, linprobe_dump = train_test_reg(
        X_train, X_test, y_train, y_test, random_state=args.seed, genes=genes, method=args.method
    )
    probe_summary = {}
...
def merge_fold_results(arr):
    ...
    for key, value in aggr_dict.items():
        aggr_results.append({"name": key, "pearson_corrs": value, "mean": np.mean(value), "std": np.std(value)})
        all_corrs += value
...
def predict_folds(args, exp_save_dir, model_name, dataset_name, device, bench_data_root, custom_encoder):
    ...
    split_dir = os.path.join(
        bench_data_root, args.custom_split_folder
    )  # NOTE, used to be splits or splits_rebalance_wo_INT20_INT22_INT23
    ...
    for i in range(n_splits):
        ...
        linprobe_results = predict_single_split(
            train_split,
            test_split,
            args,
            kfold_save_dir,
    ...
    with open(os.path.join(exp_save_dir, "results_kfold.json"), "w") as f:
...
def benchmark(
    encoder: torch.nn.Module, enc_transf: Callable, precision: torch.dtype, cli_args: dict = None, **kwargs
) -> Tuple[list, dict]:
    ...
    snapshot_download(
        repo_id="MahmoodLab/hest-bench", repo_type="dataset", local_dir=args.weights_root, allow_patterns=["fm_v1/*"]
    )

    logger.info("Fetch the bench data...")
    snapshot_download(
        repo_id="MahmoodLab/hest-bench",
        repo_type="dataset",
        local_dir=args.bench_data_root,
    ...
    dataset_perfs, perf_per_enc = benchmark_grid(
        args, device, encoders, datasets, save_dir=save_dir, custom_encoder=custom_encoder
    )
